            ...
            <tbody>
                @foreach($brands as $brand)
                    <tr>
                        ...
                        <td class="px-6 py-4 border-b">{{ Str::limit($brand->description, 50) }}</td>
                        <td class="px-6 py-4 border-b">{{ $brand->products_count }}</td>
                        <td class="px-6 py-4 border-b">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $brand->status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $brand->status ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 border-b">
                            <a href="{{ route('brands.edit', $brand) }}" class="text-blue-500 hover:text-blue-700 mr-2">Edit</a>
