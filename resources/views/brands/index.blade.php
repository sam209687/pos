        ...
        <table class="min-w-full">
            ...
            <tbody>
                @foreach($brands as $brand)
                    <tr>
                        <td class="px-6 py-4 border-b dark:border-gray-700 text-gray-900 dark:text-gray-100">{{ $brand->name }}</td>
                        <td class="px-6 py-4 border-b dark:border-gray-700 text-gray-900 dark:text-gray-100">{{ Str::limit($brand->description, 50) }}</td>
                        <td class="px-6 py-4 border-b dark:border-gray-700 text-gray-900 dark:text-gray-100">{{ $brand->products_count }}</td>
                        <td class="px-6 py-4 border-b dark:border-gray-700">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $brand->status ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-100' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-100' }}">
                                {{ $brand->status ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 border-b dark:border-gray-700">
                            <a href="{{ route('brands.edit', $brand) }}" class="text-blue-500 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 mr-2">Edit</a>
